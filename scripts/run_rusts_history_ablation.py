...
CANDIDATES = {
    "geracl_remaining": "data/contrastive/open_ru_1r_nc_mixb_geracl_remaining_9600.jsonl",
...
HISTORY_NOTES = {
    "geracl_remaining": "Best broad Mix-H continuation used this as stage 2 and kept high RuSTS.",
    "grandmaster": "Grandmaster appeared in high-RuSTS Mix-H runs and is broad semantic/category supervision.",
    "geracl": "Pure 1R-NC GeRaCl and GeRaCl-heavy runs historically scored well on RuSTS.",
    "mixh_habrfull": "The strongest clean reinit family for RuSTS used this full stage-1 mixture.",
    "deepvk_hnp": "DeepVK-HNP is a control: it helped low-step a050 recovery but not clearly STS.",
}
...
def write_config(name: str, data_path: str, args: argparse.Namespace, seed: int) -> Path:
    config = {
        "name": name,
        "description": (
            "History-driven RuSTS recovery ablation from Habr 1x + DeepVK 1x "
            "a050-recovery checkpoint."
        ),
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=1411)
    ...
    for index, candidate in enumerate(args.candidates):
        ...
        name = f"cedr_a050_habr1x_deepvk1x_rusts_hist_{candidate}_{suffix}_4096_eager_frozenrepro"
        ...
        row = {"candidate": candidate, "note": HISTORY_NOTES[candidate], **scores}
        rows.append(row)
        if not args.keep_checkpoints and checkpoint.exists():
            checkpoint.unlink()

    summary_path = RESULTS_ROOT / f"cedr_a050_habr1x_deepvk1x_rusts_history_{suffix}_gate5_summary.md"
    lines = [
        "# RuSTS History-Driven Add-On Ablations",
        "",
        (
            "Base checkpoint: "
        ...
        "",
        "| Candidate | Reason from history |",
        "|---|---|",
    ]
    for candidate in args.candidates:
        lines.append(f"| `{candidate}` | {HISTORY_NOTES[candidate]} |")
    lines.extend(
        [
            "",
            "## Gate Results",
